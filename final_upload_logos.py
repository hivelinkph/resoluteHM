...
# Company name to UUID mapping (from database)
COMPANY_IDS = {
    ...
    "Wagmi": "1856022c-5945-4999-bc88-ffa8aec2560f",
...
def upload_logo(file_path, company_name, bpo_id):
    """Upload logo to Supabase Storage and update database"""
    try:
        # Create storage path
        ...
        # Upload to storage (with upsert to replace if exists)
        try:
            ...
            if 'already exists' in str(e).lower():
                supabase.storage.from_('bpo-assets').remove([storage_path])
                supabase.storage.from_('bpo-assets').upload(
                    storage_path,
                    file_content,
                    {'content-type': f'image/{file_ext[1:]}'}
...
def main():
    """Main upload process"""
    print("=" * 80)
    print("HIMAP Logo Uploader - Final Version")
    print("=" * 80)
    ...
    print(f"\n📊 Processing {len(mappings_to_process)} logos...\n")
    ...
    for idx, mapping in enumerate(mappings_to_process, 1):
        ...
        if not bpo_id:
            print(f"  ⚠️  Company not in database - skipping")
            skipped += 1
            continue
        ...
        if not file_path.exists():
            print(f"  ⚠️  Logo file not found - skipping")
            skipped += 1
            continue
        
        # Upload
        if upload_logo(file_path, company_name, bpo_id):
            processed += 1
        else:
            skipped += 1
    
    print(f"\n{'='*80}")
    print(f"✅ Successfully uploaded {processed} logos")
    print(f"⚠️  Skipped {skipped} logos")
    print(f"{'='*80}")
    print(f"\n🎉 Done! Check your HIMAP directory page to see the logos!")
